...
class DepartSupervisorAction extends RestfulAction[Supervisor], ProjectSupport, ImportSupport[Supervisor], ExportSupport[Supervisor] {
  ...
  private def getLevels(): Seq[SupervisingLevel] = {
    val q = OqlBuilder.from(classOf[SupervisingLevel], "l")
    q.where("l.schoolOnly=true").cacheable()
    entityDao.search(q)
  }
  ...
  override protected def getQueryBuilder: OqlBuilder[Supervisor] = {
    ...
    query.where("supervisor.level in(:levels)", getLevels())
    query
  }
  ...
  def downloadTemplate(): View = {
    ...
    val levels = getLevels().map(x => x.code + " " + x.name)
