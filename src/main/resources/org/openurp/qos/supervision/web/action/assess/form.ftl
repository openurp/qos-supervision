...
[/@]
 <div class="container-fluid" id="data" width="100%" align="center" cellpadding="0" cellspacing="0">
   ...
   [@b.form name="supervingForm" id="supervingForm"  action="!save" theme="html"]
    ...
    <tr>
      ...
      <td width='50%' id="f_assessOn">听课日期：
        <select name="schedule">
          ...
          <option value="${schedule.date}_${schedule.units}" [#if (schedule.date)?string('yyyy-MM-dd')=(supervision.assessOn?string('yyyy-MM-dd'))!"--"]selected="selected"[/#if]>${schedule.date} ${schedule.room} ${schedule.teacherNames!}</option>
          [/#list]
        </select>
        小节:
            ...
            <td rowspan="${typeFields[typeName]?size}">
              [#if op.maxScore != op.minScore]
                <input type="number"  name="field_${field.id}_score" id="field_${field.id}_${op_index}_score" placeholder="${op.minScore}~${op.maxScore}" min="${op.minScore}" max="${op.maxScore}"
                  [#if op.contains(fieldScore)] style="width:100px" value="${fieldScore}"[#else]style="width:100px;display:none"[/#if]/>
